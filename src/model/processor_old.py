...
def process_input_text(instruction, model_backbone, text=None, add_video_token=False, add_image_token=False):
    ...
    if model_backbone == "internvideo2":
        return text
    elif model_backbone in [GME, LamRA, LamRA_QWEN2_5,OMNI_EMBED]:
        if text:
...
#     # --- HARDEN: Qwen2.5-Omni processor requires each sample to be a Python str ---
#     norm_texts = []
#     for t in documents_texts:
#         if t is None:
#             t = ""
#         # some versions return list[str] (or tuple) instead of str
#         if isinstance(t, (list, tuple)):
#             t = "".join("" if x is None else (x if isinstance(x, str) else str(x)) for x in t)
#         elif not isinstance(t, str):
#             t = str(t)
#         norm_texts.append(t)

#     documents_texts = norm_texts

#     # 2) Extract mm info aligned with documents
#     audio, images, videos = process_mm_info(documents, use_audio_in_video=False)

#     # 3) Processor call, batched
#     text_kwargs = dict(
#         truncation=True,
#         padding=True,
...
#     )
#     audio_kwargs = dict(max_length=2048000)
...
#     # Match other backbones: ensure ids/masks are int64
...
def OmniEmbed_process_fn(model_inputs: dict, processor, max_length=204800):
    ...
    # 1) Render chat prompts in batched mode (matches working example semantics)
    documents_texts = processor.apply_chat_template(
        conversations,
        add_generation_prompt=False,
        tokenize=False,
    )
    # Some versions may return a single string for batch size 1
    if isinstance(documents_texts, str):
        documents_texts = [documents_texts]

    # Contract checks (not hacky; they prevent silent misalignment)
    assert isinstance(documents_texts, list) and all(isinstance(x, str) for x in documents_texts)
    assert len(documents_texts) == len(conversations)

    # 2) Extract mm info aligned with the SAME conversations structure
...
process_vlm_inputs_fns = {
    ...
    OMNI_EMBED: OmniEmbed_process_fn,
}
